...
Parallax.Each = class {
	...
	animationFrame() {
		...
		if (positionParent.top < 30 && positionParent.bottom > -30) {
			...
			switch (centerPoint) {
				// верхней точке (начало родителя сталкивается с верхним краем экрана)
				case "top":
					this.offset = -1 * topToWindow;
					break;
				// в центре экрана (середина родителя внутри экрана)
				case "center":
	...
	parameters(el, parameters) {
		if (parameters.axis == "v") {
			...
			el.style.transform = `translate3D(${(parameters.direction * (this.value / parameters.coefficient)).toFixed(
				2
			)}px,0,0) ${parameters.additionalProperties}`;
		}
	}
};
